...
Extracts linear transfer parameters (S-parameters) for a general
...
\argument{DATAFORMATT=<RI|MA|DB>} Format for the S-parameter data

\begin{description}
\item[\tt RI] Real-imaginary format \\
The data is output as the real and imaginary parts for each
extracted S-parameter.  This is the default.

\item[\tt MA] Magnitude-angle format \\
The data is output as the magnitude and the phase angle of each
extracted S-parameter.  For compatibility with Touchstone formats,
the angle values are in degrees.  (Note: The phase angles output
by the \texttt{VP()} operator for \texttt{.PRINT AC} are in radians.)

\item[\tt DB] Magnitude(dB)-angle format \\
The data is output as the magnitude (in dB) and the phase angle of
each extracted S-parameter.  For compatibility with Touchstone
...
the extracted S-parameter data.  So, a netlist can have multiple
\texttt{.LIN} lines with different values for the \texttt{DATAFORMAT}
and \texttt{FILE} arguments on each line.  However, in that case, the
\texttt{SPARCALC} values must be the same on all of the \texttt{.LIN}
lines.
...
not supported.  In addition, Xyce only only S-parameter data via the
\texttt{.LIN} line.  It does not output H-parameter or Y-parameter data.
...
No output occurs from any .PRINT AC line in the netlist if
\texttt{SPARCALC=1}.
