...
\begin{description}
  \item[\tt AVG] Computes the arithmetic mean of {\tt <variable>} for
    ...
    measures, and with {\tt FROM} and {\tt TO} for {\tt AC}, {\tt DC} and
    {\tt NOISE} measures.
  ...
  \item[\tt DERIV] Computes the derivative of {\tt <variable>} at a
    ...
    {\tt DC} and {\tt NOISE} measures. The {\tt MINVAL} qualifier is used as a
...
\item[\tt ERRx] Calculates the error between two simulation variables, where
  ...
  \item[\tt INTEG] Calculates the integral of {\tt outVal} through
    ...
    {\tt FROM} and {\tt TO} for {\tt AC}, {\tt DC} and {\tt NOISE} measures.
    For HSPICE compatibility, {\tt INTEGRAL} is an allowed synonym for
    {\tt INTEG}.
  ...
  \item[\tt WHEN] Returns the time when {\tt <variable>} reaches {\tt
    ...
    for {\tt TRAN} measures, and with {\tt FROM} and {\tt TO} for {\tt AC},
    {\tt DC} and {\tt NOISE} measures. The qualifier {\tt MINVAL} acts as
    a tolerance for the comparison.  For example when {\tt <variable\(\sb{2}\)>}
