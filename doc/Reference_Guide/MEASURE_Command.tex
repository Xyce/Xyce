...
In general, information on the measurement window, the time(s) that the measure's value(s) were
...
The second mode is termed ``absolute''. In this mode, \Xyce{} attempts to 
...
For a DC measure, one would use \texttt{OUTPUT=SV} instead of \texttt{OUTPUT=TIME}.
...
As a final note, the \texttt{ERROR} measure can enable the use of \Xyce{} simulation 
...
Additional syntax differences between {\tt TRIG} and {\tt TARG} clauses in \Xyce{}
