...
\item \texttt{<lead abbreviation>} is a single character designator for
  ...
  (gate), s (source), and b (bulk).  SOI and CMG transistors have: d,
  g, s, and e (bulk), and level 10 (BSIM-SOI 3) MOSFETS have b (body).
  For PDE devices, the nodes are numbered according to the order they
  appear, so lead currents are referenced like I1(\texttt{<device>}),
  I2(\texttt{<device>}), etc.  In \Xyce{}, a \texttt{.PRINT} line
  request like \texttt{I(Q1)} is a parsing error for a multi-terminal
  device.  Instead, an explicit lead current designator
  like \texttt{IC(Q1)} must be used.
