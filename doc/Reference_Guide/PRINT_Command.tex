...
\item Wildcards are partially supported on \texttt{.PRINT}
  lines. \texttt{V(*)} will print all of the node voltages in
  the circuit for all analysis modes.  The \texttt{I(*)}, \texttt{P(*)}
  and \texttt{W(*)} wildcards are supported for TRAN and DC analyses.
  \texttt{I(*)} will print all of the currents, including both
  ...
  inductance device, and the lead currents that associated with most
  other devices.  Wildcards are also supported for the lead currents for
  the multi-terminal J, M and Z devices via \texttt{IB(*)}, \texttt{ID(*)},
  \texttt{IG(*)} and \texttt{IS(*)} and for the multi-terminal Q device
  via \texttt{IB(*)}, \texttt{IC(*)}, \texttt{IE(*)} and \texttt{IS(*)}.
  The \texttt{IE(*)} wildcard is also supported for SOI devices.  A
  request for \texttt{I(*)} will not return any of the lead currents for
  the J, M, Q or Z devices.  Finally, wildcards of the form \texttt{I1(*)},
  for devices such as PDE devices and the T device, that use numerical
  designtors are not supported yet.
