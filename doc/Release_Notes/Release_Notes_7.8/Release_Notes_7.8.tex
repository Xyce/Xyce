...
\begin{XyceItemize}
\item The PSP103 model has been modified so that the DTA parameter can be specified on the instance line.  Previously it was exclusively a model parameter.
\end{XyceItemize}
...
\item The \texttt{.GLOBAL} statement now supports multiple global nodes on the same line.  Previously each specified node needed its own unique \texttt{.GLOBAL} statement.
