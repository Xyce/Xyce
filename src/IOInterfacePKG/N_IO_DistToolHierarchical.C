...
//-----------------------------------------------------------------------------
//
// Purpose        : Defines the DistToolHierarchical class.  Distribution tool
...
#include <N_IO_DistToolHierarchical.h>
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::DistToolHierarchical
// Purpose       : ctor
// Special Notes :
// Scope         : public
// Creator       :
// Creation Date :
//-----------------------------------------------------------------------------
DistToolHierarchical::DistToolHierarchical(
    ...
    subcircuitParams_()
{
  ...
  {
    // Now create a subcircuit hierarchy, including prefixes and numbers of devices for balancing.
    circuitContext_->generateHierarchy( subcktTree_ );

    int pdc = subcktTree_.numDevices / numProcs_ + 1;
    subcktTree_.currentNode = &subcktTree_;  // Reset current node to the root of the tree.

    // Let's map the subcircuits to processors based on device count
    procToSubckt_.resize( numProcs_ );
    procToNDev_.resize( numProcs_, pdc );
    int cp = 0;
    distributeSubcircuits( cp );
  }
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::distributeSubcircuits
// Purpose       : Perform subcircuit-focused device distribution
// Special Notes :
// Scope         : public
// Creator       :
// Creation Date :
//-----------------------------------------------------------------------------
void DistToolHierarchical::distributeSubcircuits( int& cp )
{
  if (subcktTree_.childNodes.size())
  {
    std::vector<Xyce::IO::SubcktNode*>::iterator it = subcktTree_.currentNode->childNodes.begin();
    std::vector<Xyce::IO::SubcktNode*>::iterator it_end = subcktTree_.currentNode->childNodes.end();
    for ( ; it != it_end; it++ )
    {
      if ( (*it)->numDevices < procToNDev_[cp] )
      {
        std::cout << "Assigning " << (*it)->prefix << " to processor " << cp << std::endl;
        procToSubckt_[cp].push_back( (*it)->prefix );
        procToNDev_[cp] -= (*it)->numDevices;
      }
      else if ( (*it)->childNodes.size() )
      {
        // set currentNode to the childNode and recurse to add more subckts to this processor.
        std::cout << "Subckt " << (*it)->prefix << " is too large to assign to processor " << cp << ", recursing down. " << std::endl;
        subcktTree_.currentNode = *it;
        distributeSubcircuits( cp );
      }
      else if ( (cp < (numProcs_-1)) && (procToSubckt_[cp].size()) )
      {
        // assign it to the next processor
        std::cout << "Assigning " << (*it)->prefix << " to NEXT processor " << cp+1 << std::endl;
        cp++;
        procToSubckt_[cp].push_back( (*it)->prefix );
        procToNDev_[cp] -= (*it)->numDevices;
      }
      ...
      {
        // assign it to this processor
        std::cout << "Assigning " << (*it)->prefix << " to THIS processor (" << (*it)->numDevices << " > " << procToNDev_[cp] << ") " << cp << std::endl;
        procToSubckt_[cp].push_back( (*it)->prefix );
        procToNDev_[cp] -= (*it)->numDevices;
      }
    }

    //Recurse back up the tree after visiting all the children. 
    subcktTree_.currentNode = subcktTree_.currentNode->parentNode;
  }
}

//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::circuitDeviceLine
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolHierarchical::circuitDeviceLine(TokenVector & deviceLine )
{
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::endDeviceLines()
...
//-----------------------------------------------------------------------------
void DistToolHierarchical::endDeviceLines()
{
  if (Parallel::is_parallel_run(pdsCommPtr_->comm()))
  {
    if (currProc_ > 0)
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::circuitStart
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolHierarchical::circuitStart( std::string const & subcircuitName,
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::circuitEnd
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolHierarchical::circuitEnd()
{
  if (Parallel::is_parallel_run(pdsCommPtr_->comm()))
  {
    if( currProc_ != 0 )
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::setFileName
...
//-----------------------------------------------------------------------------
void DistToolHierarchical::setFileName(std::string const & fileNameIn)
{
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::distributeDevices()
...
//-----------------------------------------------------------------------------
void DistToolHierarchical::distributeDevices()
{
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::packSubcircuitData
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolHierarchical::packSubcircuitData( std::string const & subcircuitName,
                                           std::vector<std::string> const & nodes,
                                           std::string const & prefix,
                                           std::vector<Device::Param> const & params )
{
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::receiveCircuitData
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DistToolHierarchical::receiveCircuitData()
{
  if (Parallel::is_parallel_run(pdsCommPtr_->comm()))
  {
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::send
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::processDeviceBuffer
...
//-----------------------------------------------------------------------------
bool
DistToolHierarchical::processDeviceBuffer()
{
  if (Parallel::is_parallel_run(pdsCommPtr_->comm()))
  {
    ...
    for (ib=0 ; ib<bufs_.size() ; ++ib)
    {
      ...
      while( pos < bsize )
      {
        ...
        switch( lineType )
        {
          ...
          {
            Report::DevelFatal().in("DistToolHierarchical::processDeviceBuffer")
...
//-----------------------------------------------------------------------------
// Function      : DistToolHierarchical::broadcastGlobalData
...
//-----------------------------------------------------------------------------
bool
DistToolHierarchical::broadcastGlobalData()
{
  return DistToolBase::broadcastGlobalData();
}

//--------------------------------------------------------------------------
// Function      : DistToolHierarchical::parseIncludeFile
// Purpose       : Jump to include file for 2nd pass
// Special Notes :
// Creator       : Rob Hoekstra
// Creation Date : 08/02/2004
//--------------------------------------------------------------------------
bool DistToolHierarchical::parseIncludeFile(std::string const& includeFile,
             const std::string &libSelect)
{
  ...
  //Move file to location of library, if a library is being selected.
  //If there is no valid bookmark, or the string is empty, this will be the top of the file.
  //ssfPtr_->setToLibraryBookmark( libSelect );
  //ssfPtr_->setLocation(0);
  //sfPtr_->setLineNumber( 1 );
...
//--------------------------------------------------------------------------
// Function      : DistToolHierarchical::expandSubcircuitInstance
...
//--------------------------------------------------------------------------
bool DistToolHierarchical::expandSubcircuitInstance(
  ...
  std::vector<std::string> & libInside)
{
  ...
  {
    Report::UserError0().at(subcircuitInstance.getLocation())
  ...
  { // The subcircuit is in an include file.
    ...
    {
      Report::UserError().at(subcircuitInstance.getLocation())
  ...
  if( circuitContext_->haveMutualInductances() )
  {
    ...
    for( int i = 0; i < n; ++i )
    {
      ...
      // DistToolHierarchical::instantiateDevices() performs this step
      if( !circuitDeviceLine( circuitContext_->getMILine( i ) ) )
      {
        handleDeviceLine( circuitContext_->getMILine( i ), libSelect, libInside );
      }
