...
// expressions/params/etc.  This version of the function sets them one 
// parameter at a time, and is thus very slow for complex PDKs.
//-----------------------------------------------------------------------------
...
// Special Notes : AGAUSS, GAUSS, AUNIF, UNIF, RAND and LIMIT
//
//  Note: this function is now obsolete.  See the new function, 
//  setParameterRandomExpressionTerms2
//
// Scope         : public
// Creator       : Eric Keiter
// Creation Date : 7/30/2020
    ...
    if (global_param_it != global_parameter_map.end())
    {
      ...
      else
      {
        ...
        if ((*global_param_it).second != value)
        {
          ...
          if ( !(globals.deviceEntityDependVec[globalIndex].empty()) )
          {
            std::vector<entityDepend>::iterator it = globals.deviceEntityDependVec[globalIndex].begin();
            std::vector<entityDepend>::iterator end = globals.deviceEntityDependVec[globalIndex].end();
            for ( ; it != end; ++it)
            {
              entityDepend & ed = (*it);
              bool globalParamChangedLocal=true;
              ...
              if (ed.entityPtr->updateGlobalAndDependentParameters(
                    globalParamChangedLocal,
                    timeChangedLocal,
                    freqChangedLocal,
                    ed.parameterVec))
              {
                ed.entityPtr->processParams();
                ed.entityPtr->processInstanceParams();
              }
            }
          }
        }
