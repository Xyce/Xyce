c
c	vbic_3T_et_cf Solver
c
c	This program is a stand-alone FORTRAN solver for vbic_3T_et_cf.
...
c
	...
	double precision G(9,9),C(9,9),xxlast(3)
...
20	continue
	...
	if(arg.EQ.'-h')then
		write(6,*) ''
		write(6,*) 'vbic_3T_et_cf_solveri: solver for model vbic_3T_et_cf'
		write(6,*) ''
		write(6,*) 'Usage: vbic_3T_et_cf_solveri [options] [ParameterFileName]'
		...
		write(6,*) '				 vbic_3T_et_cf defaults)'
		write(6,*) ''
		write(6,*) 'Options:'
		...
		write(6,*) 'For vbic_3T_et_cf_solveri bias is current for the base node'
		write(6,*) 'The vbic_3T_et_cf solution is then printed, the first row of'
...
c
	call vbic_3T_et_cf_t(p,pnom,TAMB)
c
...
10	continue
	...
	Vrth=x(7)
	Vith=x(7)
      call vbic_3T_et_cf_fj(p,Vrth,Vbei,Vbex,Vbci,Vbep,Vrcx,Vbcx
     +,Vrci,Vrbx,Vrbi,Vre,Vrbp,Vbe,Vbc,Vcei,Ibe,Ibe_Vrth,Ibe_Vbei,Ibex
     +,Ibex_Vrth,Ibex_Vbex,Itzf,Itzf_Vrth,Itzf_Vbei,Itzf_Vbci,Itzr
     +,Itzr_Vrth,Itzr_Vbci,Itzr_Vbei,Ibc,Ibc_Vrth,Ibc_Vbci,Ibc_Vbei,Ibep
     ...
     +,Ith_Vbci,Ith_Vcei,Ith_Vbex,Ith_Vbep,Ith_Vrcx,Ith_Vrci,Ith_Vbcx
     +,Ith_Vrbx,Ith_Vrbi,Ith_Vre,Ith_Vrbp,Qcth,Qcth_Vrth,SCALE)
	do 30 ii=1,8
		f(ii)=0.0d0
		do 25 jj=1,8
			gdc(ii,jj)=0.0d0
...
c
	...
	gdc(4,7)=gdc(4,7)-Ibe_Vrth
	gdc(4,4)=gdc(4,4)-Ibe_Vbei
	gdc(4,5)=gdc(4,5)+Ibe_Vbei
	f(5)=f(5)+Ibe
	gdc(5,7)=gdc(5,7)+Ibe_Vrth
...
c
	...
	gdc(3,7)=gdc(3,7)-Ibex_Vrth
	gdc(3,3)=gdc(3,3)-Ibex_Vbex
	gdc(3,5)=gdc(3,5)+Ibex_Vbex
	f(5)=f(5)+Ibex
	gdc(5,7)=gdc(5,7)+Ibex_Vrth
...
c
	...
	gdc(3,7)=gdc(3,7)-Ibep_Vrth
	gdc(3,3)=gdc(3,3)-Ibep_Vbep
	gdc(3,6)=gdc(3,6)+Ibep_Vbep
	f(6)=f(6)+Ibep
	gdc(6,7)=gdc(6,7)+Ibep_Vrth
...
c
	...
	gdc(8,8)=gdc(8,8)-Irbx_Vrbx
	gdc(8,3)=gdc(8,3)+Irbx_Vrbx
	gdc(8,7)=gdc(8,7)-Irbx_Vrth
	f(3)=f(3)+Irbx
	gdc(3,8)=gdc(3,8)+Irbx_Vrbx
	gdc(3,3)=gdc(3,3)-Irbx_Vrbx
	gdc(3,7)=gdc(3,7)+Irbx_Vrth
c
...
c
	f(7)=f(7)-Irth
	gdc(7,7)=gdc(7,7)-Irth_Vrth
c
c	Stamp element: Ith
c
	f(7)=f(7)-Ith
	gdc(7,7)=gdc(7,7)-Ith_Vrth
	gdc(7,4)=gdc(7,4)-Ith_Vbei
	gdc(7,5)=gdc(7,5)+Ith_Vbei
	gdc(7,4)=gdc(7,4)-Ith_Vbci
	gdc(7,2)=gdc(7,2)+Ith_Vbci
	gdc(7,2)=gdc(7,2)-Ith_Vcei
	gdc(7,5)=gdc(7,5)+Ith_Vcei
	gdc(7,3)=gdc(7,3)-Ith_Vbex
	gdc(7,5)=gdc(7,5)+Ith_Vbex
	gdc(7,3)=gdc(7,3)-Ith_Vbep
	gdc(7,6)=gdc(7,6)+Ith_Vbep
	gdc(7,1)=gdc(7,1)+Ith_Vrcx
	gdc(7,1)=gdc(7,1)-Ith_Vrci
	gdc(7,2)=gdc(7,2)+Ith_Vrci
	gdc(7,4)=gdc(7,4)-Ith_Vbcx
	gdc(7,1)=gdc(7,1)+Ith_Vbcx
	gdc(7,8)=gdc(7,8)-Ith_Vrbx
	gdc(7,3)=gdc(7,3)+Ith_Vrbx
	gdc(7,3)=gdc(7,3)-Ith_Vrbi
	gdc(7,4)=gdc(7,4)+Ith_Vrbi
	gdc(7,5)=gdc(7,5)+Ith_Vre
	gdc(7,6)=gdc(7,6)-Ith_Vrbp
	gdc(7,1)=gdc(7,1)+Ith_Vrbp
c
c	Stamp element: ib
c
	f(8)=f(8)+xx(2)
...
c	Stamp element: Ibep
c
	G(3,7)=G(3,7)-Ibep_Vrth
	G(3,3)=G(3,3)-Ibep_Vbep
	G(3,6)=G(3,6)+Ibep_Vbep
	G(6,7)=G(6,7)+Ibep_Vrth
...
c
	...
	G(9,7)=G(9,7)-Ircx_Vrth
	Gc(1)=Gc(1)-Ircx_Vrcx
	G(1,1)=G(1,1)-Ircx_Vrcx
	G(1,7)=G(1,7)+Ircx_Vrth
c
c	Stamp element: Irci
c
	G(1,1)=G(1,1)-Irci_Vrci
	G(1,2)=G(1,2)+Irci_Vrci
	G(1,7)=G(1,7)-Irci_Vrth
...
c
	...
	G(8,7)=G(8,7)-Irbx_Vrth
	Gb(3)=Gb(3)-Irbx_Vrbx
	G(3,3)=G(3,3)-Irbx_Vrbx
	G(3,7)=G(3,7)+Irbx_Vrth
c
c	Stamp element: Irbi
c
	G(3,3)=G(3,3)-Irbi_Vrbi
	G(3,4)=G(3,4)+Irbi_Vrbi
	G(3,7)=G(3,7)-Irbi_Vrth
...
c	Stamp element: Irth
c
	G(7,7)=G(7,7)-Irth_Vrth
c
c	Stamp element: Ith
c
	G(7,7)=G(7,7)-Ith_Vrth
	G(7,4)=G(7,4)-Ith_Vbei
	G(7,5)=G(7,5)+Ith_Vbei
	G(7,4)=G(7,4)-Ith_Vbci
	G(7,2)=G(7,2)+Ith_Vbci
	G(7,2)=G(7,2)-Ith_Vcei
	G(7,5)=G(7,5)+Ith_Vcei
	G(7,3)=G(7,3)-Ith_Vbex
	G(7,5)=G(7,5)+Ith_Vbex
	G(7,3)=G(7,3)-Ith_Vbep
	G(7,6)=G(7,6)+Ith_Vbep
	G(7,1)=G(7,1)+Ith_Vrcx
	G(7,1)=G(7,1)-Ith_Vrci
	G(7,2)=G(7,2)+Ith_Vrci
	G(7,4)=G(7,4)-Ith_Vbcx
	G(7,1)=G(7,1)+Ith_Vbcx
	G(7,3)=G(7,3)+Ith_Vrbx
	G(7,3)=G(7,3)-Ith_Vrbi
	G(7,4)=G(7,4)+Ith_Vrbi
	G(7,5)=G(7,5)+Ith_Vre
	G(7,6)=G(7,6)-Ith_Vrbp
	G(7,1)=G(7,1)+Ith_Vrbp
c
c	Stamp element: Qbe
c
	C(4,7)=C(4,7)-Qbe_Vrth
	C(4,4)=C(4,4)-Qbe_Vbei
	C(4,5)=C(4,5)+Qbe_Vbei
	C(4,4)=C(4,4)-Qbe_Vbci
	C(4,2)=C(4,2)+Qbe_Vbci
	C(5,7)=C(5,7)+Qbe_Vrth
