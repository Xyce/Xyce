c
c	vbic_4T_et_xf Solver
c
c	This program is a stand-alone FORTRAN solver for vbic_4T_et_xf.
c	For usage see the help message that is printed below.
c
	implicit double precision (a-z)
	double precision p(108),pnom(108)
	double precision xx(4),cx(4),vmx(5),imx(4)
	double precision x(11),xlast(11),xinit(11)
	double precision G(12,12),C(12,12),xxlast(4)
	double precision Ginv(12,12),Glu(12,12),B(12,12)
	double precision BGinv(12,12),Geff(12,12)
	double precision Frb(12),Fib(12),Frc(12),Fic(12)
	double precision fr(12),fi(12)
	integer i,j,k,nit,nac,iargc,nargs,acc,ierr,iinit,doac,dodc
	integer ipvt(12),ipvtGlu(12)
...
20	continue
	...
	if(arg.EQ.'-h')then
		write(6,*) ''
		write(6,*) 'vbic_4T_et_xf_solveri: solver for model vbic_4T_et_xf'
		write(6,*) ''
		write(6,*) 'Usage: vbic_4T_et_xf_solveri [options] [ParameterFileName]'
		...
		write(6,*) '				 vbic_4T_et_xf defaults)'
		write(6,*) ''
		write(6,*) 'Options:'
		...
		write(6,*) '	c b e s dt [f for -yp option]'
		write(6,*) 'For vbic_4T_et_xf_solveri bias is current for the base node'
		write(6,*) 'The vbic_4T_et_xf solution is then printed, the first row of'
...
c
	call vbic_4T_et_xf_t(p,pnom,TAMB)
c
...
140			continue
	...
	if(doac.EQ.0)then
		if(acc.EQ.0)then
			write(6,201) (vmx(i),i=1,5),nit
			write(6,201) (imx(i),i=1,4)
		else
			write(6,202) (vmx(i),i=1,5),nit
...
199	continue
	...
	call exit(1)
200	continue
201	format(5(1x,1pe11.3),i4)
202	format(5(1x,1pe30.20),i4)
	call exit(0)
	end
c
c	Newton solver
c
	...
	implicit double precision (a-z)
	integer n,i,j,k,ii,jj,itmax,nit,ipvt(11)
	integer errflg,nfail
	double precision p(108),xx(4),vmx(5),imx(4)
	double precision x(11),f(11),gdc(11,11)
	double precision G(12,12),C(12,12),xxlast(4)
	double precision Gb(12),Cb(12),Gc(12),Cc(12)
	itmax=200
	dvmax=0.2d0
	dtmax=10.0d0
	vatol=1.0d-8
...
c		cx ci bx bi ei bp si xf1 xf2 [dt|b|dt b]
c
5	continue
10	continue
	...
	Vlxf=x(8)-x(9)
	Vrxf=x(9)
      call vbic_4T_et_xf_fj(p,Vrth,Vbei,Vbex,Vbci,Vbep,Vbcp,Vrxf
     +,Vrcx,Vbcx,Vrci,Vrbx,Vrbi,Vre,Vrbp,Vrs,Vbe,Vbc,Vcei,Vcep,Vcxf,Ibe
     +,Ibe_Vrth,Ibe_Vbei,Ibex,Ibex_Vrth,Ibex_Vbex,Itxf,Itxf_Vrxf,Itzr
     +,Itzr_Vrth,Itzr_Vbci,Itzr_Vbei,Ibc,Ibc_Vrth,Ibc_Vbci,Ibc_Vrxf
     +,Ibc_Vbei,Ibep,Ibep_Vrth,Ibep_Vbep,Ircx,Ircx_Vrcx,Ircx_Vrth,Irci
     +,Irci_Vrci,Irci_Vrth,Irci_Vbci,Irci_Vbcx,Irbx,Irbx_Vrbx,Irbx_Vrth
     +,Irbi,Irbi_Vrbi,Irbi_Vrth,Irbi_Vbei,Irbi_Vbci,Ire,Ire_Vre,Ire_Vrth
     +,Irbp,Irbp_Vrbp,Irbp_Vrth,Irbp_Vbep,Irbp_Vbci,Qbe,Qbe_Vrth
     +,Qbe_Vbei,Qbe_Vbci,Qbex,Qbex_Vrth,Qbex_Vbex,Qbc,Qbc_Vrth,Qbc_Vbci
     +,Qbcx,Qbcx_Vrth,Qbcx_Vbcx,Qbep,Qbep_Vrth,Qbep_Vbep,Qbep_Vbci,Qbeo
     +,Qbeo_Vbe,Qbco,Qbco_Vbc,Ibcp,Ibcp_Vrth,Ibcp_Vbcp,Iccp,Iccp_Vrth
     +,Iccp_Vbep,Iccp_Vbci,Iccp_Vbcp,Irs,Irs_Vrs,Irs_Vrth,Qbcp,Qbcp_Vrth
     +,Qbcp_Vbcp,Irth,Irth_Vrth,Ith,Ith_Vrth,Ith_Vbei,Ith_Vbci,Ith_Vrxf
     +,Ith_Vcei,Ith_Vbex,Ith_Vbep,Ith_Vrs,Ith_Vbcp,Ith_Vcep,Ith_Vrcx
     +,Ith_Vrci,Ith_Vbcx,Ith_Vrbx,Ith_Vrbi,Ith_Vre,Ith_Vrbp,Qcth
     +,Qcth_Vrth,Ixzf,Ixzf_Vrth,Ixzf_Vbei,Ixzf_Vbci,Ixxf,Ixxf_Vrxf,Qcxf
     +,Qcxf_Vcxf,Flxf,Flxf_Vrxf,SCALE)
	do 30 ii=1,11
		f(ii)=0.0d0
		do 25 jj=1,11
			gdc(ii,jj)=0.0d0
...
c
	...
	gdc(4,10)=gdc(4,10)-Ibe_Vrth
	gdc(4,4)=gdc(4,4)-Ibe_Vbei
	gdc(4,5)=gdc(4,5)+Ibe_Vbei
	f(5)=f(5)+Ibe
	gdc(5,10)=gdc(5,10)+Ibe_Vrth
...
c
	...
	gdc(3,10)=gdc(3,10)-Ibex_Vrth
	gdc(3,3)=gdc(3,3)-Ibex_Vbex
	gdc(3,5)=gdc(3,5)+Ibex_Vbex
	f(5)=f(5)+Ibex
	gdc(5,10)=gdc(5,10)+Ibex_Vrth
...
c
	...
	gdc(2,9)=gdc(2,9)-Itxf_Vrxf
	f(5)=f(5)+Itxf
	gdc(5,9)=gdc(5,9)+Itxf_Vrxf
c
...
c
	...
	gdc(3,10)=gdc(3,10)-Ibep_Vrth
	gdc(3,3)=gdc(3,3)-Ibep_Vbep
	gdc(3,6)=gdc(3,6)+Ibep_Vbep
	f(6)=f(6)+Ibep
	gdc(6,10)=gdc(6,10)+Ibep_Vrth
...
c
	...
	gdc(11,11)=gdc(11,11)-Irbx_Vrbx
	gdc(11,3)=gdc(11,3)+Irbx_Vrbx
	gdc(11,10)=gdc(11,10)-Irbx_Vrth
	f(3)=f(3)+Irbx
	gdc(3,11)=gdc(3,11)+Irbx_Vrbx
	gdc(3,3)=gdc(3,3)-Irbx_Vrbx
	gdc(3,10)=gdc(3,10)+Irbx_Vrth
c
...
c
	...
	gdc(7,10)=gdc(7,10)-Ibcp_Vrth
	gdc(7,7)=gdc(7,7)-Ibcp_Vbcp
	gdc(7,6)=gdc(7,6)+Ibcp_Vbcp
	f(6)=f(6)+Ibcp
	gdc(6,10)=gdc(6,10)+Ibcp_Vrth
...
c
	...
	gdc(7,10)=gdc(7,10)+Irs_Vrth
c
c	Stamp element: Irth
c
	f(10)=f(10)-Irth
	gdc(10,10)=gdc(10,10)-Irth_Vrth
c
c	Stamp element: Ith
c
	f(10)=f(10)-Ith
	gdc(10,10)=gdc(10,10)-Ith_Vrth
	gdc(10,4)=gdc(10,4)-Ith_Vbei
	gdc(10,5)=gdc(10,5)+Ith_Vbei
	gdc(10,4)=gdc(10,4)-Ith_Vbci
	gdc(10,2)=gdc(10,2)+Ith_Vbci
	gdc(10,9)=gdc(10,9)-Ith_Vrxf
	gdc(10,2)=gdc(10,2)-Ith_Vcei
	gdc(10,5)=gdc(10,5)+Ith_Vcei
	gdc(10,3)=gdc(10,3)-Ith_Vbex
	gdc(10,5)=gdc(10,5)+Ith_Vbex
	gdc(10,3)=gdc(10,3)-Ith_Vbep
	gdc(10,6)=gdc(10,6)+Ith_Vbep
	gdc(10,7)=gdc(10,7)+Ith_Vrs
	gdc(10,7)=gdc(10,7)-Ith_Vbcp
	gdc(10,6)=gdc(10,6)+Ith_Vbcp
	gdc(10,3)=gdc(10,3)-Ith_Vcep
	gdc(10,7)=gdc(10,7)+Ith_Vcep
	gdc(10,1)=gdc(10,1)+Ith_Vrcx
	gdc(10,1)=gdc(10,1)-Ith_Vrci
	gdc(10,2)=gdc(10,2)+Ith_Vrci
	gdc(10,4)=gdc(10,4)-Ith_Vbcx
	gdc(10,1)=gdc(10,1)+Ith_Vbcx
	gdc(10,11)=gdc(10,11)-Ith_Vrbx
	gdc(10,3)=gdc(10,3)+Ith_Vrbx
	gdc(10,3)=gdc(10,3)-Ith_Vrbi
	gdc(10,4)=gdc(10,4)+Ith_Vrbi
	gdc(10,5)=gdc(10,5)+Ith_Vre
	gdc(10,6)=gdc(10,6)-Ith_Vrbp
	gdc(10,1)=gdc(10,1)+Ith_Vrbp
c
...
c
	...
	gdc(8,10)=gdc(8,10)-Ixzf_Vrth
	gdc(8,4)=gdc(8,4)-Ixzf_Vbei
	gdc(8,5)=gdc(8,5)+Ixzf_Vbei
	gdc(8,4)=gdc(8,4)-Ixzf_Vbci
	gdc(8,2)=gdc(8,2)+Ixzf_Vbci
c
...
c
	f(8)=f(8)-x(9)
	gdc(8,9)=gdc(8,9)-1.0d0
	f(9)=f(9)+x(8)
	gdc(9,8)=gdc(9,8)+1.0d0
c
...
c	Stamp element: Irbx
c
	Gb(11)=Gb(11)+Irbx_Vrbx
	G(11,3)=G(11,3)+Irbx_Vrbx
	G(11,10)=G(11,10)-Irbx_Vrth
	Gb(3)=Gb(3)-Irbx_Vrbx
	G(3,3)=G(3,3)-Irbx_Vrbx
	G(3,10)=G(3,10)+Irbx_Vrth
c
c	Stamp element: Irbi
c
	G(3,3)=G(3,3)-Irbi_Vrbi
	G(3,4)=G(3,4)+Irbi_Vrbi
	G(3,10)=G(3,10)-Irbi_Vrth
...
c
	G(5,5)=G(5,5)-Ire_Vre
	G(5,10)=G(5,10)+Ire_Vrth
c
c	Stamp element: Irbp
c
	G(6,6)=G(6,6)-Irbp_Vrbp
	G(6,1)=G(6,1)+Irbp_Vrbp
	G(6,10)=G(6,10)-Irbp_Vrth
...
c
	G(7,7)=G(7,7)-Irs_Vrs
	G(7,10)=G(7,10)+Irs_Vrth
c
c	Stamp element: Irth
c
	G(10,10)=G(10,10)-Irth_Vrth
c
c	Stamp element: Ith
c
	G(10,10)=G(10,10)-Ith_Vrth
	G(10,4)=G(10,4)-Ith_Vbei
	G(10,5)=G(10,5)+Ith_Vbei
	G(10,4)=G(10,4)-Ith_Vbci
	G(10,2)=G(10,2)+Ith_Vbci
	G(10,9)=G(10,9)-Ith_Vrxf
	G(10,2)=G(10,2)-Ith_Vcei
	G(10,5)=G(10,5)+Ith_Vcei
	G(10,3)=G(10,3)-Ith_Vbex
	G(10,5)=G(10,5)+Ith_Vbex
	G(10,3)=G(10,3)-Ith_Vbep
	G(10,6)=G(10,6)+Ith_Vbep
	G(10,7)=G(10,7)+Ith_Vrs
	G(10,7)=G(10,7)-Ith_Vbcp
	G(10,6)=G(10,6)+Ith_Vbcp
	G(10,3)=G(10,3)-Ith_Vcep
	G(10,7)=G(10,7)+Ith_Vcep
	G(10,1)=G(10,1)+Ith_Vrcx
	G(10,1)=G(10,1)-Ith_Vrci
	G(10,2)=G(10,2)+Ith_Vrci
	G(10,4)=G(10,4)-Ith_Vbcx
	G(10,1)=G(10,1)+Ith_Vbcx
	G(10,3)=G(10,3)+Ith_Vrbx
	G(10,3)=G(10,3)-Ith_Vrbi
	G(10,4)=G(10,4)+Ith_Vrbi
	G(10,5)=G(10,5)+Ith_Vre
	G(10,6)=G(10,6)-Ith_Vrbp
	G(10,1)=G(10,1)+Ith_Vrbp
c
c	Stamp element: Ixzf
c
	G(8,10)=G(8,10)-Ixzf_Vrth
	G(8,4)=G(8,4)-Ixzf_Vbei
	G(8,5)=G(8,5)+Ixzf_Vbei
	G(8,4)=G(8,4)-Ixzf_Vbci
	G(8,2)=G(8,2)+Ixzf_Vbci
c
c	Stamp element: Ixxf
c
	G(9,9)=G(9,9)-Ixxf_Vrxf
c
c	Stamp element: Qbe
c
	C(4,10)=C(4,10)-Qbe_Vrth
	C(4,4)=C(4,4)-Qbe_Vbei
	C(4,5)=C(4,5)+Qbe_Vbei
	C(4,4)=C(4,4)-Qbe_Vbci
	C(4,2)=C(4,2)+Qbe_Vbci
	C(5,10)=C(5,10)+Qbe_Vrth
