c
c	vbic_3T_it_cf Solver
c
c	This program is a stand-alone FORTRAN solver for vbic_3T_it_cf.
...
c
	...
	double precision x(6),xlast(6),xinit(6)
	double precision G(8,8),C(8,8),xxlast(3)
	double precision Ginv(8,8),Glu(8,8),B(8,8)
	double precision BGinv(8,8),Geff(8,8)
	double precision Frb(8),Fib(8),Frc(8),Fic(8)
	double precision fr(8),fi(8)
	integer i,j,k,nit,nac,iargc,nargs,acc,ierr,iinit,doac,dodc
	integer ipvt(8),ipvtGlu(8)
...
20	continue
	...
	if(arg.EQ.'-h')then
		write(6,*) ''
		write(6,*) 'vbic_3T_it_cf_solverv: solver for model vbic_3T_it_cf'
		write(6,*) ''
		write(6,*) 'Usage: vbic_3T_it_cf_solverv [options] [ParameterFileName]'
		...
		write(6,*) '				 vbic_3T_it_cf defaults)'
		write(6,*) ''
		write(6,*) 'Options:'
		...
		write(6,*) 'For vbic_3T_it_cf_solveri bias is current for the base node'
		write(6,*) 'The vbic_3T_it_cf solution is then printed, the first row of'
...
c
	call vbic_3T_it_cf_t(p,pnom,TAMB)
c
...
c
	...
	implicit double precision (a-z)
	integer n,i,j,k,ii,jj,itmax,nit,ipvt(6)
	integer errflg,nfail
	double precision p(108),xx(3),vmx(3),imx(3)
	double precision x(6),f(6),gdc(6,6)
	double precision G(8,8),C(8,8),xxlast(3)
	double precision Gb(8),Cb(8),Gc(8),Cc(8)
	itmax=200
	dvmax=0.2d0
	dtmax=10.0d0
	vatol=1.0d-8
...
10	continue
	...
	Vrbi=x(3)-x(4)
	Vrbp=x(6)-x(1)
      call vbic_3T_it_cf_fj(p,Vbei,Vbex,Vbci,Vbep,Vrcx,Vbcx,Vrci
     +,Vrbx,Vrbi,Vre,Vrbp,Vbe,Vbc,Ibe,Ibe_Vbei,Ibex,Ibex_Vbex,Itzf
     ...
     +,Qbep_Vbci,Qbeo,Qbeo_Vbe,Qbco,Qbco_Vbc,SCALE)
	do 30 ii=1,6
		f(ii)=0.0d0
		do 25 jj=1,6
			gdc(ii,jj)=0.0d0
