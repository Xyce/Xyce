c
c	vbic_3T_it_xf Solver
c
c	This program is a stand-alone FORTRAN solver for vbic_3T_it_xf.
c	For usage see the help message that is printed below.
c
	implicit double precision (a-z)
	double precision p(108),pnom(108)
	double precision xx(3),cx(3),vmx(3),imx(3)
	double precision x(9),xlast(9),xinit(9)
	double precision G(10,10),C(10,10),xxlast(3)
...
20	continue
	...
	if(arg.EQ.'-h')then
		write(6,*) ''
		write(6,*) 'vbic_3T_it_xf_solveri: solver for model vbic_3T_it_xf'
		write(6,*) ''
		write(6,*) 'Usage: vbic_3T_it_xf_solveri [options] [ParameterFileName]'
		...
		write(6,*) '				 vbic_3T_it_xf defaults)'
		write(6,*) ''
		write(6,*) 'Options:'
		...
		write(6,*) '	c b e [f for -yp option]'
		write(6,*) 'For vbic_3T_it_xf_solveri bias is current for the base node'
		write(6,*) 'The vbic_3T_it_xf solution is then printed, the first row of'
...
c
	call vbic_3T_it_xf_t(p,pnom,TAMB)
c
...
140			continue
	...
	if(doac.EQ.0)then
		if(acc.EQ.0)then
			write(6,201) (vmx(i),i=1,3),nit
			write(6,201) (imx(i),i=1,3)
		else
			write(6,202) (vmx(i),i=1,3),nit
...
199	continue
	...
	call exit(1)
200	continue
201	format(3(1x,1pe11.3),i4)
202	format(3(1x,1pe30.20),i4)
	call exit(0)
	end
c
c	Newton solver
c
	...
	double precision p(108),xx(3),vmx(3),imx(3)
...
10	continue
	...
	Vlxf=x(7)-x(8)
	Vrxf=x(8)
      call vbic_3T_it_xf_fj(p,Vbei,Vbex,Vbci,Vbep,Vrxf,Vrcx,Vbcx
     +,Vrci,Vrbx,Vrbi,Vre,Vrbp,Vbe,Vbc,Vcxf,Ibe,Ibe_Vbei,Ibex,Ibex_Vbex
     +,Itxf,Itxf_Vrxf,Itzr,Itzr_Vbci,Itzr_Vbei,Ibc,Ibc_Vbci,Ibc_Vrxf
     ...
     +,Qbep_Vbci,Qbeo,Qbeo_Vbe,Qbco,Qbco_Vbc,Ixzf,Ixzf_Vbei,Ixzf_Vbci
     +,Ixxf,Ixxf_Vrxf,Qcxf,Qcxf_Vcxf,Flxf,Flxf_Vrxf,SCALE)
	do 30 ii=1,9
		f(ii)=0.0d0
		do 25 jj=1,9
			gdc(ii,jj)=0.0d0
